...
import { addVendorContactApiSchema } from "@/features/vendor/types";
...
export async function GET(request: NextRequest) {
    try {
        ...
        const contacts = await db
            ...
            .from(contact)
            .where(eq(contact.category, "VENDOR"))
...
export async function POST(request: NextRequest) {
    try {
        ...
        const validatedData = addVendorContactApiSchema.safeParse(body);
        ...
        console.error('Error creating new vendor contact: ', error);
        return NextResponse.json(
            { error: "Internal server error" },
            { status: 500 }
        );
